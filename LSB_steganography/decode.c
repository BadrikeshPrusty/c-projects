                ...
                if ( decode_secret_file_extn(decInfo->extn_file, decInfo) == e_success )    //function call to decode file extension
                {
                    ...
                    if ( decode_secret_file_size(decInfo->size_secret_file, decInfo) == e_success ) //function call to decode secret file size
                    {
                        ...
                        if ( decode_secret_file_data(decInfo) == e_success )        //function call to decode data and store it in file
                        {
                            ...
                            fclose(decInfo->fptr_stego_image);          //closing all the opened files
...
Status decode_magic_string(const char *magic_string, DecodeInfo *decInfo)
{
    ...
    for ( int i = 0; i < 2; i++ )                       //iterate loop for 2 times 1 for # and then for *
    {
        fread(buffer, 1, 8, decInfo->fptr_stego_image); //take 8 bytes from file to buffer
        ch = 0;                                         //initialise to 0 before performing bitwise operations

        for ( int j = 0; j < 8; j++ )                   //iterate loop 8 times to get the LSB bits from the buffers
        {
            ch |= (buffer[j] & 1) << (7 - j);           //store the LSB bits of buffer from MSB to LSB of ch var one by one
        }

        if ( magic_string[i] != ch )                    //if char doesn't match with index value of magic string return e_failure
        {
            return e_failure;                           //indicationg that match failure
        }
    }

    return e_success;                                   //if matches return e_failure
}
